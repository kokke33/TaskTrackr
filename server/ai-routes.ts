...
上記の分析結果または会話履歴に対する、ユーザーの質問、または指示内容に丁寧で具体的な回答をしてください。
質問の場合、分析結果にない情報については推測せず、「分析結果からは判断できません」と回答してください。回答は簡潔で実用的にしてください。
指示内容の場合は、指示に従って具体的なアクションを提案してください。`;
