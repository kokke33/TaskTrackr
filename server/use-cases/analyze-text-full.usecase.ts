...
export async function analyzeTextFull(
  ...
  aiLogger.logDebug(providerName, 'analyzeTextFull', requestId, 'Starting two-stage text analysis', { textLength: content.length, fieldType, provider: providerName }, userId);

  try {
    // ========== 第1段階: 詳細分析 ==========
    aiLogger.logDebug(providerName, 'analyzeTextFull', requestId, 'Starting stage 1: Detailed analysis', { stage: 1 }, userId);
    ...
    aiLogger.logDebug(providerName, 'analyzeTextFull', requestId, 'Stage 1 completed', { 
      stage: 1, 
    ...
    // ========== 第2段階: エグゼクティブサマリ生成 ==========
    aiLogger.logDebug(providerName, 'analyzeTextFull', requestId, 'Starting stage 2: Executive summary generation', { stage: 2 }, userId);

    const executiveSummary = await generateExecutiveSummary(
      aiProvider, firstStageResult, content, fieldType, userId, requestId
    );

    aiLogger.logDebug(providerName, 'analyzeTextFull', requestId, 'Two-stage analysis completed successfully', { 
      stage: 2,
    ...
    aiLogger.logError(providerName, 'analyzeTextFull', requestId, error as Error, userId, { text: content });
    
    const fallback = "申し訳ございませんが、現在AI分析サービスに接続できません。しばらく後に再度お試しください。";
    
    aiLogger.logDebug(providerName, 'analyzeTextFull', requestId, 'Using fallback analysis result', { fallback }, userId);
    return fallback;
  }
}
...
`,
  ...
  const response = await aiProvider.generateResponse(messages, userId, { operation: 'analyzeTextFull-stage1', text: content });
...
A5: [分析結果に基づく回答]`
  ...
  const response = await aiProvider.generateResponse(messages, userId, { 
    operation: 'analyzeTextFull-stage2-executiveSummary', 
    fieldType,
