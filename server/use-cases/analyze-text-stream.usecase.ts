...
`,
    ...
    if (aiProvider.supportsStreaming && aiProvider.generateStreamResponse) {
      // ストリーミング対応プロバイダーの場合
      for await (const chunk of aiProvider.generateStreamResponse(messages, userId, { operation: 'analyzeTextStream', text: content })) {
        yield chunk;
      }
    } else {
      // 非ストリーミングプロバイダーの場合：一括で送信
      const response = await aiProvider.generateResponse(messages, userId, { operation: 'analyzeTextStream', text: content });
      const cleanedContent = response.content.replace(/<think>[\s\S]*?<\/think>/g, '').trim();
      yield cleanedContent;
    }

    aiLogger.logDebug(providerName, 'analyzeTextStream', requestId, 'Text analysis stream completed successfully', { analysisCompleted: true }, userId);
