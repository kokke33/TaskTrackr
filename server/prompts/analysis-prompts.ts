...
・競合他社がAI機能を標準搭載開始
・市場全体でセキュリティ要求レベルが向上`,
  [ANALYSIS_FIELD_TYPES.adminConfirmationEmail]: `
【必須レイアウト】
件名: [明確で簡潔な件名]
...
■確認事項
[週次報告の内容を踏まえた具体的な確認項目を番号付きで記載]

■回答期限
[具体的な期限と理由]

【記載例】
件名: 週次報告について確認事項があります
...
■確認事項
1. システム設計書の進捗が80%とありますが、残り20%の内容と完了予定はいつ頃でしょうか？
2. 外部API連携で発生した問題の詳細と、顧客への影響範囲を教えてください
3. 来週の性能テストで懸念されているレスポンス時間の目標値を確認したいです
...
【分析目的】
正しいレイアウトで記載内容のレベルを上げることが目的です。「現在の記載内容」を中心にと「前回報告の内容からの変更点」を考慮し、以下の点に注意して分析を行ってください。

【分析指示】
1. 現在の記載内容が正しいレイアウトに準拠しているかチェックしてください
2. レイアウトに問題がある場合は、正しいレイアウトでの記載例を提示してください
3. 前回報告内容は考慮しつつ、現在の内容を中心に分析してください。前回報告内容を再度報告する必要はありません。
4. 記載内容のレベルアップのため、より具体的で有用な内容への改善提案を行ってください{{layoutRequirements}}{{isContentUnchanged}}`;